...
import PrintButton from "@/components/PrintButton" // Asegúrate que la ruta coincida con donde creaste el componente
...
export default async function ListaInicialPage() {
    ...
    const estudiantes = await prisma.estudiante.findMany({
        where: {
            ...
            nivel: "INICIAL" // Para Primaria, cambia esto a "PRIMARIA"
        },
        orderBy: [
            { gradoOEdad: 'asc' },
            { apellidos: 'asc' }
            ...
            {gradosUnicos.length === 0 ? (
                ...
                <div className="space-y-8">
                    {gradosUnicos.map((grado) => {
                        ...
                        // Generamos un ID seguro para HTML reemplazando espacios con guiones
                        const tableId = `tabla-inicial-${grado.replace(/\s+/g, '-')}`
                            ...
                            <div key={grado} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                                ...
                                <div className="bg-blue-50 border-b border-blue-100 p-4 flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <PrintButton targetId={tableId} title={`Nivel Inicial - ${grado}`} />
